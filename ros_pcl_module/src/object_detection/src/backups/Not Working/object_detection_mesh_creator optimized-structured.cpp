...
private:
    void pointCloudCallback(const sensor_msgs::msg::PointCloud2::SharedPtr msg) {
        ...
        if (cloud_filtered->points.empty()) {
            RCLCPP_WARN(this->get_logger(), "Gefilterte Punktwolke ist leer nach NaN-Entfernung.");
            return;
        }
        ...
        if (cloud_filtered->points.empty()) {
            RCLCPP_WARN(this->get_logger(), "Gefilterte Punktwolke ist leer nach Ebenenentfernung.");
            return;
        }
        ...
        for (const auto &indices : cluster_indices) {
            ...
            if (mesh.polygons.empty()) {
                RCLCPP_WARN(this->get_logger(), "Keine gültigen Meshes erstellt.");
                continue;
            }
    ...
    void convertMeshToMarkers(const pcl::PolygonMesh &mesh, visualization_msgs::msg::MarkerArray &marker_array, int marker_id) {
        ...
        marker.scale.z = 1.0;
        marker.color.a = 1.0;
        marker.color.r = static_cast<float>(marker_id % 2); // Farbkodierung
