...
#include <pcl/gpu/segmentation/gpu_segmentation.h>
#include "plane_segmentation.cu"

class MarkerArrayNode : public rclcpp::Node {
public:
    MarkerArrayNode() 
        : Node("gpu_marker_array_node"),
...
private:
    ...
    */
    ...
    typename pcl::PointCloud<PointT>::Ptr planeSegmentation(
        ...
        long t1 = std::chrono::duration_cast<std::chrono::milliseconds>(
            std::chrono::system_clock::now().time_since_epoch()).count();
        
        pcl::gpu::DeviceArray<pcl::PointXYZ> cloud_device;
        cloud_device.upload(cloudIn->points);
        pcl::gpu::Segmentation seg_gpu;
        pcl::gpu::DeviceArray<int> inliers_gpu;
        seg_gpu.setInputCloud(cloud_device);
        seg_gpu.setModelType(pcl::SACMODEL_PLANE);
        seg_gpu.setMethodType(pcl::SAC_RANSAC);
        seg_gpu.setDistanceThreshold(0.02);
        seg_gpu.segment(inliers_gpu);

        if (removeNaNValues) {
            // Parallelisiertes Entfernen von NaN-Werten mit Thrust
            cloudOut->points.erase(
                thrust::remove_if(
                    thrust::device, cloudOut->points.begin(), cloudOut->points.end(),
                    [] __device__ (const PointT& p) { return isnan(p.x) || isnan(p.y) || isnan(p.z); }
                ),
                cloudOut->points.end()
            );
        }
        ...
        if (removePlanePoints) {
            // CUDA Kernel zum Entfernen der Inlier-Punkte
            launchRemovePlanePointsKernel(cloud_device, inliers_gpu);
        } else {
            for (int idx : inliers_gpu) {
        ...
        cloud_device.download(cloudOut->points);
